...
def is_analysis_running():
    if ida_auto.is_auto_enabled():
        return True  # Analysis is currently running
    if not ida_auto.wanted_actions_empty():
        return True  # There are still pending analysis actions
    return False  # Analysis is idle
...
def run():
    ...
    while curr_addr < func_end:
        ...
        if hex(prev_curr_addr) == hex(curr_addr):
            print('!!! 1 - Caught double address!!! prev_curr_addr=' + hex(prev_curr_addr) + 'hex(curr_addr)=' + hex(curr_addr))
        ...
        if (is_obfuscated):
            ...
            #for i in range(min(curr_addr, jmp_addr), max(curr_addr, jmp_addr)):
...
idaapi.compile_idc_text('static fn2() { RunPythonStatement("run2()"); }')
add_idc_hotkey("F2", 'fn2')
